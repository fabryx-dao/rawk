...
import type { Message, ClientMessage, Connection, RelayStats, Config } from './types.js';
...
export class ARCRelay {
  ...
  private agents: Map<string, WebSocket>; // agent_id → WebSocket
  private sockets: WeakMap<WebSocket, string>; // WebSocket → agent_id
  private connections: Map<string, Connection>; // agent_id → Connection

  // Subscription management
  private subscriptions: Map<string, Set<string>>; // target_agent → Set<subscriber_agents>
  ...
  async handleMessage(ws: WebSocket, data: RawData): Promise<void> {
    ...
    if (clientMsg.type) message.type = clientMsg.type;
    if (clientMsg.ref) message.ref = clientMsg.ref;
   ...
   */
  async routeMessage(msg: Message, sender_id: string): Promise<void> {
   ...
   */
  async handleRelayCommand(msg: Message, sender_id: string): Promise<void> {
    const type = msg.type;
   ...
   * Handle ping request
   */
  async handlePing(sender_id: string): Promise<void> {
   ...
   */
  async handleSubscribe(msg: Message, sender_id: string): Promise<void> {
   ...
   */
  async handleUnsubscribe(msg: Message, sender_id: string): Promise<void> {
    const agents = msg.payload?.agents;
   ...
   */
  async handleListSubscriptions(sender_id: string): Promise<void> {
    const subscribed_to: string[] = [];
   ...
   */
  async deliverToSubscribers(msg: Message, sender_id: string): Promise<void> {
   ...
   */
  async directMessage(msg: Message, targets: string[], sender_id: string): Promise<void> {
    ...
    console.log(
      `[direct] ${sender_id} → ${targets.join(', ')}: ${delivered} delivered, ${notFound.length} not connected`
    );
  }
   ...
   */
  async broadcast(msg: Message, sender_id: string): Promise<void> {
    let delivered = 0;
